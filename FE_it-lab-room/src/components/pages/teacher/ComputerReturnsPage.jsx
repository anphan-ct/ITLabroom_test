...
export default function ComputerReturnsPage() {
  ...
  const submit = async (event) => {
    ...
    if (selectedLoan && Number(form.quantity) > selectedLoan.so_luong_con_lai) {
      setFormError(`Số lượng trả vượt quá số máy chưa trả hoặc đang chờ duyệt (còn lại: ${selectedLoan.so_luong_con_lai} máy).`);
      return;
    }
            ...
            <Field label="Phiếu mượn">
              <SelectInput value={form.loanId} onChange={(loanId) => setForm({ ...form, loanId })}>
                ...
                {approvedLoans.map((request) => {
                  ...
                  if (remaining === 0) {
                    return <option key={request.id} value={request.id} disabled>{request.ma_phieu_muon} (Đã trả đủ {request.so_luong}/{request.so_luong})</option>;
                  }
                  return <option key={request.id} value={request.id}>{request.ma_phieu_muon} (Còn lại: {remaining}/{request.so_luong})</option>;
                })}
              </SelectInput>
            </Field>
