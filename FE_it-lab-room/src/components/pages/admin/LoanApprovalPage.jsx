...
function AllocationModal({ request, rooms, onClose, onSubmit }) {
  ...
  const toggleSelect = (id) => {
    setSelectedIds(prev => {
      ...
      setConditions(prevCond => ({ ...prevCond, [id]: "tot" }));
      return [...prev, id];
    });
  };
          ...
          <table className="w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {roomFilter === "unselected" ? (
                <tr><td colSpan="4" className="p-4 text-center text-slate-500">Chọn 1 phòng để xem danh sách máy</td></tr>
              ) : isLoadingComputers ? (
                <tr><td colSpan="4" className="p-4 text-center text-slate-500">Đang tải danh sách máy...</td></tr>
              ) : activeComputers.length === 0 ? (
                <tr><td colSpan="4" className="p-4 text-center">Không có máy rảnh trong phòng này</td></tr>
                    ...
                    <td className="p-3">
                      {isSelected ? (
                        ...
                        >
                          ...
                          <option value="hu_nang">Hư nặng</option>
                        </select>
                      ) : (
  ...
  const fetchData = async () => {
    try {
      ...
      const [reqRes, roomRes] = await Promise.all([
        loanRequestService.getAdminLoanRequests(statusFilter, 1), // passing statusFilter and page 1
        getRoomsFromApi()
      ]);
      if (reqRes.status) {
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              key: "actions",
              title: "Thao tác",
              render: (_, request) => request.trang_thai === "pending" ? (
                ...
                <span className="text-slate-400 font-medium text-xs uppercase">—</span>
              ),
            },
          ]}
